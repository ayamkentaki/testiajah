...
if not os.path.exists('./sessions'):
    os.mkdir('./sessions')
if not os.path.exists(f"Users/+6288297802013/phone.csv"):
   os.mkdir('./Users')
   os.mkdir(f'./Users/+6288297802013')
   open(f"Users/+6288297802013/phone.csv","w")
     ...
     try:
      ...
      with open("Users/1882918909/phone.csv", 'r')as f:
         ...
         with open('1.csv', 'w', encoding='UTF-8') as writeFile:
            writer = csv.writer(writeFile, lineterminator="\n")
            writer.writerows(NonLimited)
         with open("1.csv") as infile, open(f"Users/+6288297802013/phone.csv", "w") as outfile:
            for line in infile:
                outfile.write(line.replace(",", ""))
      os.remove("1.csv")
